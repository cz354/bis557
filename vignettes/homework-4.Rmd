...
The function we create in this question is lm_lasso_py whose codes are in the file "lm-lasso-py.r" and its test codes are in "test-lm-lasso-py.r"

We know that:
$$\hat(\beta)=sign(X^TY)(X^TX)^{-1}max\{|X^TY|-n\lambda,0\}$$
```{r}
data(iris)
Y=matrix(iris$Sepal.Length, ncol = 1)
X=model.matrix(Sepal.Length ~ .,iris)
fit_lasso=lm_lasso_py(Y,X,lambda=0.01)
...
lm_lasso_py <- function(Y,X, lambda){
  ...
  X=r_to_py(X)
  Y=r_to_py(Y)
  sign=np$sign(X$T$dot(Y))
  max=np$maximum(np$subtract(np$abs(X$T$dot(Y)),n*lambda),0)
  inv=np$linalg$inv(X$T$dot(X))
  beta = np$multiply(sign,inv$dot(max))
